...
{	
	...
	if ([m_delegate respondsToSelector:@selector(remotingCall:didReceiveResponse:)])
	{
        ...
        [m_delegate remotingCall:self didReceiveResponse:response];
	}
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
	...
	if ([m_delegate respondsToSelector:@selector(remotingCall:didReceiveData:)])
	{
        ...
        [m_delegate remotingCall:self didReceiveData:data];
	}
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
	...
	if (m_error)
	{
        ...
        [m_delegate remotingCall:self didFailWithError:m_error];
	}
	else
	{
        ...
        [m_delegate remotingCallDidFinishLoading:self receivedObject:data];
		[message release];
	}
	[self _cleanup];
	m_isLoading = NO;
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    ...
    [m_delegate remotingCall:self didFailWithError:error];
	[self _cleanup];
	m_isLoading = NO;
}
	...
	redirectResponse:(NSURLResponse *)redirectResponse
{
	...
	{
		return (NSURLRequest *)[m_delegate remotingCall:self willSendRequest:request redirectResponse:redirectResponse];
	}
	return request;
}
